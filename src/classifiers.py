    ...
    async def get_answer(self, templateId, pubid, score, topic, jaccard, entrancy, best_etalon):
        ...
        if resp["hits"]["hits"]:
            ...
            return SearchResult(templateId=search_result[0]["templateId"], templateText=search_result[0]["templateText"], topic=topic, 
                                score=score, jaccard=jaccard, entrance=entrancy, best_etalon=best_etalon).dict()
        else:
            ...
            if tokens[0]:
                ...
                if search_result["hits"]["hits"]:
                    ...
                    if etalons_search_result:
                        results_tuples = [(d["ID"], d["Cluster"], d["LemCluster"], d["Topic"], d["ShortAnswerText"]) for d in etalons_search_result[:candidates]]
                        # print(results_tuples[:15])
                        # print(len(results_tuples))
                        text_emb = self.model.encode(tokens_str)
                        ids, ets, lm_ets, topics, ans_txs = zip(*results_tuples)
                        ...
                        if self.topic_check:
                            ...
                            etalons_candidates = [(i, et, lm_et, lm_tp, sc, jaccard_similarity(lm_tp, tokens_str),  
                                                   texts_entrance(lm_tp, tokens_str), an_t) for i, et, lm_et, lm_tp, sc, an_t in
                                                zip(ids, ets, lm_ets, lm_topics, scores_list, ans_txs)  if sc >= score]
                        else:
                        ...
                        if the_best_result:
                            return {"id": the_best_result[0],
                                    "etalon": the_best_result[1],
                                    "lm_etalon": the_best_result[2],
                                    "topic": the_best_result[3],
                                    "sbert_score": the_best_result[4],
                                    "fa_text": the_best_result[7],}
                            """
